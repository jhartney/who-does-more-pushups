...
Our null hypothesis is:   
  $H_0$:  There is no relationship between the amount of pushups done and the position of the players on the football team at Georgetown College. 

Our alternate hypothesis is:   
  $H_a$: There IS a relationship between the amount of pushups done and the position of the players on the football team at Georgetown College. 

```{r}
t.test(pushups ~ position, data=pushups)
```

The P-value calculated by the t-test is approximately .000009421. Since this number is extreeeemely small, the chances of obtaining results like this in a test with potentially different subjects are very slim. Therefore, we reject the null hypothesis and favor the alternate hypothesis. This simply means we choose to believe the $H_a$ over the $H_0$ as they are stated above.  
